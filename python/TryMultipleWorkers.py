...
w.rxPlane.set_nxnynz(1, 1024, 1024)
w.rxPlane.clear_pressurefield()
...
# update the world ny based on the lines done
...
for idx_worker in range(need_workers):
    y_line_n = idx_worker * each_worker_does_y_lines  # starts at zero
    y_line_y = world.rxPlane.y0 + world.rxPlane.dy * y_line_n
    handles.append({
        'received': False,
        'y_line_y': y_line_y,
        'y_line_n': y_line_n,
        'async_handle': cueBeamCore2.beamsim_instant.delay(
            k=world.wavenumber,
            x0=world.rxPlane.x0,
            y0=y_line_y,
            z0=world.rxPlane.z0,
