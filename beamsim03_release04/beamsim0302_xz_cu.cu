...
// beamsim0302_xz_cu.cu
// this version only calculates image along xz axis, instead it does it fast.
// this is by mapping y to z.
...
//__device__ __constant__ float tx_const_xz[1536];  // tx table limited to 256 entries - takes 6*256=1536 floats, which is 6144 bytes out of 8192 available
//  __device__ 
        __constant__ float tx_const_xz[12288]; // allocate 64x64 transmitters
        ...
        float k)
{
 ...
 //   float pi=3.141592653589793f;
    ...
    // start actual calculation
        ...
        for (itx=0; itx<tx_length*6; itx=itx+6) // this hopefully acesses the same memory location for each thread and therefore will be cached
         {
            ...
            dix=(pixel_x-tx_const_xz[0+itx]);
            diy=(pixel_y-tx_const_xz[1+itx]);
            diz=(pixel_z-tx_const_xz[2+itx]);
            distance=sqrtf( dix*dix + diy*diy + diz*diz ); 

            kd=-k*distance+tx_const_xz[5+itx];
            //dist2=tx_const_xz[4+itx]/(2*pi*distance);
            dist2=tx_const_xz[4+itx]/(pi2*distance);
	   ...
	   //offset=ix+nx*iy+(ny*nx)*iz;  // this is for an xyz version
       offset=ix+nx*iz; // that's a version for xy version
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  error=cudaMemcpyToSymbol( "tx_const_xz", (float*) mxGetData(prhs[0]), tx_arraySize*sizeof(float) );
  if(error != cudaSuccess){
		mexPrintf("Error is : %s\n", cudaGetErrorString(cudaGetLastError()));
		mexErrMsgTxt("Problem transferring tx_const_xz table\n");
	}
